...
while true; do
    ...
    while [ -z "$CONSUL_ADDR" ]; do
        for addr in 10.0.0.2:8500 10.0.0.3:8500 10.0.0.4:8500 10.0.0.5:8500 10.0.0.6:8500; do
            if wget -T 3 -qO- http://$addr/v1/agent/services 2>/dev/null | grep -q "${SIDECAR_NAME}-sidecar-proxy"; then
            CONSUL_ADDR=$addr;
            break;
            fi;
        done;
    ...
    if ! wget -T 5 -qO- http://$CONSUL_ADDR/v1/status/leader 2>/dev/null | grep -q '.' ; then
        echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
